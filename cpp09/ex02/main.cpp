...
int main(int argc, char **argv) {
    ...
    for (int j = 1; argv[j]; j++) {
        std::istringstream iss(argv[j]);//handle eof
        int n;
    ...
    //handle 1 element
    if (victor.size() == 1) {
        std::cout << "Final main: " << victor[0] << std::endl;
        return 0;
    }
    ...
    std::vector<std::pair<int,int> > cupido = makePend(victor, cmp);//add cmp TEST
    int strangler = makeStrang(victor);

    testCupido(cupido); //TEST
    testStrangler(strangler);//TEST
        
    // Recursion
    std::vector<int> mainChain = makeMain(cupido);
    // fordJohnson(mainChain, cupido, strangler, 0);
    fordJohnson(mainChain, cupido, strangler, 0, cmp);
